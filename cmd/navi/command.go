...
func (cmd *ProjectCommand) executeCommand(ctx Ctx, watchData *ExecuteWatchData, isAfterCmd, isMainCommand bool) error {
	...
	for idx, cmdTokens := range cmd.Tokens {
		originalCmd := cmd.OriginalCmds[idx]
		...
		logger.InfoWithPrefix(cmd.GetLogPrefix(), "Executing `%s`", originalCmd)

		// Prepare command with shell wrapping if needed
		commandArgs := cmdTokens
		if strings.TrimSpace(cmd.Shell) != "" {
			commandArgs = prepareShellCommand(cmd.Shell, originalCmd)
		}

		// Execute the command
		if err := cmd.executeSingleCommand(ctx, watchData, isAfterCmd, commandArgs, originalCmd); err != nil {
			return err
		}
	}
...
func (cmd *ProjectCommand) executeSingleCommand(ctx Ctx, watchData *ExecuteWatchData, isAfterCmd bool, cmdArgs []string, originalCmd string) error {
	...
	waitForOutput, err := cmd.setupCommandOutputHandling(processCmd)
	if err != nil {
		return err
	}
	...
	if err := processCmd.Start(); err != nil {
		...
		return fmt.Errorf("Command `%s` has failed with error `%v`", originalCmd, err)
	}
	...
	if err := processCmd.Wait(); err != nil {
		...
		return fmt.Errorf("Command `%s` failed with exit code %v", originalCmd, err)
	}
...
func getProjectCommand(args []string) (projectCmd *ProjectCommand, projectNotFound bool, err error) {
	...
	if len(extraArgs) > 0 {
		lastIdx := len(projectCommand.Tokens) - 1
		projectCommand.Tokens[lastIdx] = append(projectCommand.Tokens[lastIdx], extraArgs...)

		lastIdx = len(projectCommand.OriginalCmds) - 1
		projectCommand.OriginalCmds[lastIdx] += " " + strings.Join(utils.AddQuotesToArgsWithSpaces(extraArgs), " ")
	}
...
func buildProjectCommand(
	...
	case any: // Simple command string or list
		...
		commandTokens, originalCommands, err := parseCommandStrings(commandList)
		if err != nil {
			return nil, err
		}
...
// parseCommandStrings converts command strings to tokens and preserves originals
func parseCommandStrings(commandList []string) ([][]string, []string, error) {
	tokens := make([][]string, 0, len(commandList))
	originalCmds := make([]string, 0, len(commandList))

	for _, command := range commandList {
		cmdTokens, err := shellquote.Split(command)
		if err != nil {
			return nil, nil, fmt.Errorf("Invalid format for command `%v`", command)
		}

		tokens = append(tokens, cmdTokens)
		originalCmds = append(originalCmds, command)
	}

	return tokens, originalCmds, nil
}
...
func buildCommandFromMap(
	...
	commandTokens, originalCommands, err := parseCommandStrings(cmdConfig.Run)
	if err != nil {
		return nil, err
	}
...
func (cmd *ProjectCommand) setupCommandOutputHandling(processCmd *exec.Cmd) (func(), error) {
...
// prepareShellCommand wraps a command with the appropriate shell
func prepareShellCommand(shell string, cmd string) []string {
...
func parseCommandMap(cmdData map[string]any, cmdName, projName string, isGlobalCommand bool) (CommandConfig, error) {
	...
	cmdStrings, isValidFormat := convertToStringList(rawCmd)
	if !isValidFormat {
		if isGlobalCommand {
